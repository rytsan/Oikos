...
D51	A HUD INFORMA recurso e NARRA estabilidade: comida, madeira, pedra, população, ano e era aparecem como número; decadência, tensão e legitimidade NÃO aparecem como número — voltam como sintoma diegético (fumaça, ruína, êxodo, notícia no toast)	a D3 já dizia que estabilidade nunca é número e que o mapa narra, mas a topbar da S12 imprimia "decadência N" e nem o implementador nem o auditor pegaram — o harness testa comportamento, não conformidade de design. Recurso é concreto e o jogador o gasta: sem o número não há decisão de construção. Estado interno é o que a D3 protege. A emenda que tira o contador entra na rodada 7A.1, junto do resto da tipografia, para não re-integrar o S12 duas vezes
D52	Tipografia da interface: SERIFADA LITERÁRIA, servida por Google Fonts com fallback local (permitido pela seção 3). Algarismos tabulares obrigatórios em todo número que muda por tick	a spec da S12 pede "fonte com caráter" e a implementação usava ui-monospace, que diz terminal e não crônica de civilização. Algarismos proporcionais fazem o número tremer horizontalmente a cada tick — detalhe que denuncia amadorismo e custa uma linha de CSS (font-variant-numeric: tabular-nums)
12. Status
...
v1.16	Interface: D51 (a HUD informa recurso e narra estabilidade — "decadência N" sai da tela) e D52 (tipografia serifada literária, com algarismos tabulares obrigatórios)
