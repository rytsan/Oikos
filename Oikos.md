...
Delegar → testar harness localmente → auditar → emendar (aprovado) →integrar → rodar o jogo → próxima rodada. Nunca delegar a rodada seguintecom a anterior não integrada.
...
D-variabilidade (decisão de dono aberta): alvo de terra [950, 1350] efloresta [12%, 26%] sorteados por seed via child('world'), em vez dequantil fixo (~1100/18%). Default enquanto não ratificada: manter quantil fixo.
12. Status
Módulo	Spec	Delegado	Auditado	Integrado
S1 Kernel	✅ v1.5	✅	✅ (código + harness 11/11)	✅ integrado no index.html (sessão #1)
S2 Worldgen	✅ v1.5	✅	✅ (código + harness 13/13 medido; D26/D27)	✅ integrado no index.html (sessão #1)
S3 Render	✅ v1.5 + adendo	☐	☐	☐
